...
Now, if you want to code along, you can find the branch with the starter code [here on GitHub](https://github.com/benjamin-chavez/nextjs-express-auth0-template/branches).
...
```bash
touch client/src/middleware.ts
```
...
If you log out and go back to [http://localhost:3000](http://localhost:3000), you should once again have access to both our `Home` and `Public` pages. Additionally, if you navigate to [http://localhost:3000/private](http://localhost:3000/private), you will be redirected to our login screen.
...
## Connecting our Express Server to Auth0

If you return to [http://localhost:3000/api](http://localhost:3000/api), you will see the following `Not Found` error:

<Image
  src="/img/blog/auth0-nextjs-express/ss7.png"
  width="718"
  height="404"
  alt="Screenshot"
/>

This is because Auth0 doesn't know about our Express.js Server yet. Let's add our server to Auth0 and connect it to our Next.js app inside of Auth0.
...
2. Click the `+ Create API button` and update the form's Name and Identifier fields and click create:
...
3. Update the Environment Variables
   ...
   - Copy the `AUTH0_ISSUER_BASE_URL` environment variable from `client/.env.local` and add it to `server/.env`
...
AUTH0_SECRET=replace-with-your-own-secret-generated-by-running-openssl
AUTH0_BASE_URL=http://localhost:3000
AUTH0_ISSUER_BASE_URL='https://dev-q514l5sgo7t68du7.us.auth0.com'
AUTH0_CLIENT_ID='25Y6cTzFPUAAoweiKcbc7Kd9Mc1OYnt5'
AUTH0_CLIENT_SECRET='W1XJQNmn7AmU46v_GXsQ7WxntFnA59zof5Cx4fg-VRqruQPPBe-Hbtxk9UZHucjB'
AUTH0_AUDIENCE=http://localhost:5000/api
```
...
Howerver, it is good practice to add CORS anyway so that we save our future selves from creating unnecesary bugs.

1. Copy over the `AUTH0_BASE_URL` Environment variable from your next.js app's `.env.local` file into your Express Server's `.env` file:

```bash:server/.env showLineNumbers
PORT=5000
