...
1. Create a custom middleware function and pass it as an argument to the existing `withMiddlewareAuthRequired()` function.
   ...
   The new custom middleware function should take in a `NextRequest` object, copy any existing headers over to a new `NextResponse` object.
...
2. Note that we need to import `getSession` from Auth0 so that we can access our our user session and auth token:
...
3. Remove the Authorization header from our newly created `NextResponse` object so that it is not exposed:
